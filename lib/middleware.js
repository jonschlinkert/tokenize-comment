    ...
    parser
      ...
      .set('gfm', function() {
        ...
        if (match) {
          ...
          var fenceLen = match[2].length;
          var fence = fenceLen === 3 ? '```' : '````';
          var idx = this.input.indexOf(fence);
          ...
          if (match[1]) {
            var whitespaceLen = match[1].length;
            node.val = node.val.split('\n').map(function(ele) {
              return ele.slice(whitespaceLen);
            }).join('\n');
          }
      ...
      .set('description', function() {
        var pos = this.position();
        var match = this.match(/^ {0,3}(?!@|`{3,4}| {4})[^\n]+/);
        if (match) {
          return pos(this.node({
            type: 'description',
            val: match[0]
